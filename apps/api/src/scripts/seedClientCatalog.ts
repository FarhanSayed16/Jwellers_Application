/**
 * Seed categories + CSV items + opening rates for CLIENT_SLUG.
 * Reads clients/<slug>/catalog/items.csv
 *
 *   CLIENT_SLUG=acme npm run seed:client-catalog -w @jwellers/api
 *   CLIENT_SLUG=ratnaraj npm run seed:client-catalog -w @jwellers/api
 */
...
function resolveCatalogCsv(slug: string): string {
  const candidates = [
    path.resolve(process.cwd(), `../../clients/${slug}/catalog/items.csv`),
    path.resolve(__dirname, `../../../../clients/${slug}/catalog/items.csv`),
  ];
  const file = candidates.find((p) => fs.existsSync(p));
  if (!file) throw new Error(`clients/${slug}/catalog/items.csv not found`);
  return file;
}
...
async function ensureAdminId(slug: string): Promise<string> {
  const owner =
    (await AdminUserModel.findOne({ role: 'owner', isActive: true })) ||
    (await AdminUserModel.create({
      name: `${slug} Owner`,
      email: process.env.SEED_OWNER_EMAIL ?? `owner@${slug}.jewellers.local`,
...
async function main() {
  ...
  console.log(`[seed:client-catalog] CLIENT_SLUG=${slug}`);
  ...
  const csvPath = resolveCatalogCsv(slug);
  const rows = parseCsv(fs.readFileSync(csvPath, 'utf8'));
  const adminId = await ensureAdminId(slug);
  ...
  if (!latestRate) {
    await RateModel.create({
      ...
      gold18kPerGram: 5850,
      silverPerGram: 98,
      note: `${slug} opening rate`,
      effectiveAt: new Date(),
      source: 'manual',
      createdBy: adminId,
    });
    console.log('[seed:client-catalog] created opening rates');
  }

  const itemCount = await ItemModel.countDocuments({ deletedAt: null });
  console.log('[seed:client-catalog] done', { clientSlug: slug, import: result, itemCount, csv: csvPath });
  await disconnectMongo();
}

main().catch(async (err) => {
  console.error('[seed:client-catalog] failed', err);
